...
#####
#2nd March 2016: Add flag to be able to search reference genome without 'chr' flag 
#13th April 2016: Updated to only consider heterozygous sites with at least 5X coverage in RNA data - speeds up program when using SNVs from WGS
...
if ($chr_flag) {
  print "You have specified that the reference mapped to doesn't contain \"chr\"\n";
}
...
sub InsertSNPs ($) {
  ...
  my $tempout_pos = $rna_dir."/hetbam_".$rna_id."_".$chromosome_number."_pos.txt"; #file for read identifiers
  open (POS, ">$tempout_pos") || die "Unable\n";
  my %pos_cov = (); #Hash for positions to check coverage
  my @het_array_chr = ();
  for (my $i=0;$i<@het_array;$i++) { #collect hets just in each chromosome
    if ($het_array[$i] =~ /^$chromosome\_/) {
      push @het_array_chr, $het_array[$i];
      my @a = split (/_/,$het_array[$i]);
      $pos_cov{$a[1]}=0;
      print POS "$a[1]\n"; #print out het positions for coverage check
    }
  }
  close (POS);

  my @pos_out;
  @pos_out = `samtools depth $rna_file -r $chromosome_number | grep -Fwf $tempout_pos` if ($chr_flag);
  @pos_out = `samtools depth $rna_file -r $chromosome | grep -Fwf $tempout_pos` if (!($chr_flag));
  #print "@pos_out\n";
  for (my $pp=1;$pp<@pos_out;$pp++) {
    my @a = split(/\s/,$pos_out[$pp]);
    $pos_cov{$a[1]}=$a[2]; #Get coverage of each heterozygous site in RNAseq data
  }
  ...
  for (my $i=0;$i<@het_array_chr;$i++) { #for each het pos - speed up using BED file with all positions?
    ...
    my @a = split (/_/,$pos);
    if ($pos_cov{$a[1]}>4) { #Only consider heterozygous positions that have at least 3 reads in RNA (can change, but this should improve speed for whole genome data)
      my $tag = $a[0].":".$a[1]."-".$a[1]; #create region identifier
      $tag =~ s/chr// if ($chr_flag);
  ...
  my @collected_sam = ();
  @collected_sam = `samtools view $rna_file $chromosome | grep -Fwf $tempout_tags` if (!($chr_flag)); #for all read identifiers overlapping hets, get read and read pair
  @collected_sam = `samtools view $rna_file $chromosome_number | grep -Fwf $tempout_tags` if ($chr_flag); #for all read identifiers overlapping hets, get read and read pair
...
--RemoveChr : Use this flag to specify that reference names do not include "chr" e.g >chr1 is in reference fasta as >1. Default: off (Optional)
